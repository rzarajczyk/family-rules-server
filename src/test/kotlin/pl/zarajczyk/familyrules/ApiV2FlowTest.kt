...
class ApiV2FlowTest : AbstractIntegrationTest() {

    @Autowired
    private lateinit var restTemplate: TestRestTemplate

    private val v2Client = V2Client()
    ...
    fun `should not register new instance if invalid password`() {
        ...
        // when
        val response = v2Client.registerInstance(password = "invalid")
    ...
    fun `should not register new instance if instance with this name already exists`() {
        // given
        v2Client.registerInstance(name = "testInstanceX")
        val initialCount = db.countInstances()

        // when
        val response = v2Client.registerInstance(name = "testInstanceX")
    ...
    fun `should send launch request`() {
        // given
        val i = v2Client.registerInstance().getInstanceSettings()
    ...
    private inner class V2Client {
        ...
        fun registerInstance(
            ...
            val response = restTemplate.postForEntity(
                "/api/v2/register-instance", entity, Map::class.java
            )
        ...
        fun launch(
            ...
            val headers = HttpHeaders().also {
                it.contentType = APPLICATION_JSON
                it.setBasicAuth(instance.instanceId.toString(), instance.token)
            }
            val entity = HttpEntity(request, headers)
            val response = restTemplate.postForEntity(
                "/api/v2/launch", entity, Map::class.java
            )
        ...
        fun report(
            ...
            val entity = HttpEntity(request, headers)
            val response = restTemplate.postForEntity(
                "/api/v2/report", entity, Map::class.java
            )
